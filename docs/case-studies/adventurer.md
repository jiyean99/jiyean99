...
<sub>실측 기준 - 백업 저장소를 직접 파싱해 산출(파일 수 · enum 멤버 수).
...
| **동시 담당** | 모노레포 내 저장소 6개 (서비스 프론트 · 리포트 · 홈 · 어드민 · 공용 라이브러리 등) |
...
| 포털 소켓 이벤트 | **26종** | 소켓 이벤트 enum 멤버 수 실측 |
...
RESULT
  ...
  Remote Config 250키 · 모노레포 내 저장소 6개 동시 담당
  (트래픽 지표는 회사 자산이라 미기재)
```
