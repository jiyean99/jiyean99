...
이 프로젝트의 중심 과제는 예측 정확도를 높이는 것이 아니라 **"이 데이터로 무엇을 약속할 수 있는가"를 정하는 일**이었습니다. 원천은 하루 한 번 던져지는 스냅샷이고, 시스템 간 결합에는 실제로 한계가 있었습니다(Global Tracking ↔ Port Radar 매칭률 39%, House B/L·Invoice·Container 0%).
...
<sub>실측 기준 — 백업 스냅샷 `lge-visibility`(2026-08-20) · 배포 단위는 개발원장(2026-08-10) 기준.
...
| **원천 간 결합 한계** | Global Tracking ↔ Port Radar **39%**<br/>House B/L · Invoice · Container **0%**<br/><sub>(고객사 확인 회신 2026-06)</sub> | 붙지 않는 축으로 정밀 예측을 약속할 수 없음 → **산출물 재정의** |
...
| ✅ **엔티티 키 검증** | 원천 간 조인 가능성을 실측 | 항만 키 99.6% / GT↔PR 39% / B/L·Invoice·Container 0% |
...
| **커버리지** | **98.13%** (88,754 / 90,444) | **85.56%** (77,387 / 90,444) |
...
```text
원안   기항 축(cp_vessel_leg)을 기준으로 조립
       → 붙는 범위가 5.8%밖에 안 됨

발견   화물 축(container_tracking)이 이미 pol / ts1 / ts2 / ts3 / pod 5칸과
       각각의 ATA · ATD를 갖고 있음
...
접는 규칙(`min(container_tracking_id)`)은 **설정 행 하나에 두고 러너와 서빙 뷰가 같은 값을 읽게** 했습니다. 두 곳에 각각 적으면 반드시 갈립니다.
...
ETA3(B 트랙)는 통짜라 구간을 쪼개지 않으므로 `no_leg_path` · `no_baseline_leg`를 쓸 수 없습니다. 그리고 B가 값을 못 내는 압도적 다수는 **"모델에 넣을 입력행이 아예 없다"**였습니다 — 90,444건 중 **13,057건(14.44%)**.

여기에 `not_applicable`을 쓰면 **"원래 대상이 아니다"로 읽혀서, "원천을 붙이면 값이 나온다"는 사실이 사라집니다.** 그래서 `no_model_input`이라는 이름을 따로 만들었습니다.

원인은 계보 차이입니다 — 모델 원천은 `core.lead_time_raw`인데 A가 선 `core.container_tracking`과 적재 계보가 달라 커버리지가 어긋납니다.
...
> **제가 한 일은 배선입니다.** 오프라인에서 `pandas 2.3.3` · `scikit-learn 1.7.2`(pkl 학습 버전과 동일) · `holidays 0.102`를 맞춰 넣고, 초기화 46.8초 + 예측 77,387건 327.9초(건당 4.2ms)로 값을 채웠습니다.
>
...
Result    동일 파일 중복 적재 0건 (누적 약 20만 행)
...
```text
Problem   매칭률 39% · 0%인 데이터로 정밀 ETA를 약속해야 하는가?
...
```mermaid
erDiagram
  INGEST_RUN ||--o{ CONTAINER_TRACKING : "적재 단위"
  INGEST_RUN ||--o{ ETA_OBSERVATION : "일 캡처"
  CONTAINER_TRACKING ||--o{ ETA2_TRACK_VALUE : "B/L · 기항"
  CONTAINER_TRACKING }o--o| PORT_RADAR : "항만 키"
  PORT_RADAR ||--o{ PORT_RADAR_SNAPSHOT : "스냅샷"
...
### 조인 일치율 — 정확히 무엇을 측정한 수치인가

> ⚠️ **99.6%는 `port_radar` ↔ `port_radar_snapshot` 항만 키 한 쌍의 일치율이며, 전체 조인이 아닙니다.**
> 잔여 불일치는 원천의 부분정보 분할 방출이 원인임을 규명하고 대체 판정 로직을 제안했습니다.
>
> 전체 결합에는 한계가 있었습니다 — Global Tracking ↔ Port Radar **39%**, House B/L · Invoice · Container **0%** (고객사 확인 회신 2026-06).
...
<sub>관련 커밋 — `bbd2335` · `83ea7a2`</sub>

### Challenge 3 — 조인 축이 5.8%밖에 안 붙는다

```text
Problem
  ETA 산출에 필요한 기항 목록을 기항 축(cp_vessel_leg)에서 가져오려 했는데
  화물과 붙는 범위가 5.8%였다.

Investigation
  다른 테이블의 컬럼 구조를 전수 확인.
  화물 축(container_tracking)이 이미 pol/ts1/ts2/ts3/pod 5칸과
  각각의 ATA·ATD 를 갖고 있음을 발견.
...
| **멱등성 검증** | 동일 파일 재적재 시 skip 확인 — 중복 적재 0건 (누적 약 20만 행) |
...
| 항목 | 값 | 확인 방법 |
|---|---|---|
| ETA2 커버리지 | 0 → **98.13%** (88,754 / 90,444) | 개발원장 작업현황 2026-08-09 |
| ETA3 커버리지 | 0 → **85.56%** (77,387 / 90,444) | 〃 |
| 조인 배치 | 2h 8m(미완료) → **15초** | 커밋 `bbd2335` · `83ea7a2` |
| 조인 축 커버리지 | 5.8% → **99.9%** | 실측 (90,361 / 90,444) |
| AIS 무응답 감지 | 13시간 미감지 → **30초** | 헬스 계층 재설계 |
| 중복 재적재 | **0건** (누적 약 20만 행) | `uq_ingest_run_kind_sha256` |
...
```text
BEFORE  ─ 제품 정의
  "정확한 도착시각 1개"를 약속 → 매칭률 39%·0% 데이터로는 감당 불가
...
**39%·0% 한계를 더 일찍 확인했어야 합니다.** 프로파일링을 먼저 했더라면 산출물 재정의가 더 앞당겨졌을 것이고, 그만큼 다른 데 시간을 쓸 수 있었습니다.
...
**데이터가 안 붙는다는 사실 자체가 산출물이 될 수 있다는 것.** 39%·0%는 실패 보고처럼 보이지만, 고객사 입장에서는 "어느 키를 정비해야 하는가"라는 정보였습니다.
...
```text
PROBLEM
  하루 한 번 스냅샷과, 서로 붙지 않는 원천(39% · 0%)으로
