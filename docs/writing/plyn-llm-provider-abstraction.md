...
`PLYN` · `2026.06 – 2026.08` · [플랫폼 →](https://plynai.com) · [프로젝트 상세 →](../experience.md#plyn)
...
이 구조 위에서 공급사 도메인 9종을 REST 엔드포인트 32종으로 묶어 하나의 실행 시스템으로 구성했고, 이 리스크 예측·설명 흐름을 대외 데모로도 확장했다. 문서 기반 수집 파이프라인을 단일 흐름으로 묶어 리스크 컨텍스트를 2분 내 확보하는 데모 시나리오도 이 프로바이더 추상화 위에서 만들어졌다.
